...
char* report_file_name = "results3b.txt";
...
int main(int argc, char** argv) {
    ...
    if (handle == NULL)
    {
        fprintf(stderr, "Couldn't open dynamic library!!!");
        return -1;
    }

    char*** (*create_table)(int table_size) = dlsym(handle, "create_table");

    int* (*create_block_sizes)(int table_size) = dlsym(handle, "create_block_sizes");

    void (*wc_files)(int files_num, char** file_names) = dlsym(handle, "wc_files");

    void (*wc_files_to_tmp)(int files_num, char** file_names) = dlsym(handle, "wc_files_to_tmp");

    int (*save_data_from_tmp_to_table)(int table_size, char*** table, int* block_sizes, int rows_num) = dlsym(handle, "save_data_from_tmp_to_table");

    void (*delete_block)(int table_size, char*** table, int* block_sizes, int id) = dlsym(handle, "delete_block");

    void (*delete_table)(int* table_size, char*** table, int* block_sizes) = dlsym(handle, "delete_table");

    int (*wc_rows_num)(int files_num) = dlsym(handle, "wc_rows_num");
