...
void send_signals(pid_t catcher_pid, int mode, int signal1, int signal2) {
    if (mode == SIGQUEUE) {
        ...
        for (int i=0; i < initial_sig_num; ++i) {
            printf("Sender - sending signal 1 #%d using SIGQUEUE\n", i + 1);
            sigqueue(catcher_pid, signal1, val);
            sigsuspend(&pass_2_signals_mask);
        }
        printf("Sender - sending signal 2 using SIGQUEUE\n");
        ...
        for (int i=0; i < initial_sig_num; ++i) {
            printf("Sender - sending signal 1 #%d using KILL\n", i + 1);
            kill(catcher_pid, signal1);
            sigsuspend(&pass_2_signals_mask);
        }
        printf("Sender - sending signal 2 using KILL\n");
...
void handler(int sig, siginfo_t *info, void *ucontext) {
    if (info->si_code == SI_QUEUE) {
        if(info->si_value.sival_int == -1) return;
    }
    if (sig == SIGUSR1 || sig == SIGRTMIN) {
        ...
        printf("\n");
        union sigval confirmation_val;
        confirmation_val.sival_int = -1;
        sigqueue(info->si_pid, sig, confirmation_val);
        sigsuspend(&pass_2_signals_mask);
    }
